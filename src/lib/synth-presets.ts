...
export const SYNTH_PRESETS: Record<string, SynthPreset> = {
  ...
  // #ЗАЧЕМ: Совместимость с V2 именами для Лаборатории.
  bass_jazz_warm: {
    layers: [{ type: 'sine', detune: 0, octave: 0, gain: 1.0 }],
    adsr: { attack: 0.02, decay: 0.2, sustain: 0.8, release: 0.4 },
    filter: { type: 'lowpass', cutoff: 600, q: 1.0 },
    lfo: { shape: 'sine', rate: 0, amount: 0, target: 'pitch' },
    effects: { distortion: 0, chorus: { rate: 0, depth: 0, mix: 0 }, delay: { time: 0, feedback: 0, mix: 0 } }
  },
  bass_ambient: {
  ...
  organ_soft_jazz: {
    layers: [
      ...
      { type: 'sine', detune: 2, octave: 1, gain: 0.4 },
    ],
    adsr: { attack: 0.05, decay: 0.1, sustain: 0.9, release: 0.4 },
    filter: { type: 'lowpass', cutoff: 3500, q: 1 },
    lfo: { shape: 'sine', rate: 6.0, amount: 4, target: 'pitch' },
    effects: {
      distortion: 0,
      chorus: { rate: 0.4, depth: 0.003, mix: 0.3 },
      delay: { time: 0, feedback: 0, mix: 0 },
    },
  },
  ...
  mellotron: {
    ...
    lfo: { shape: 'sine', rate: 4.5, amount: 5, target: 'pitch' }, 
    effects: {
      distortion: 0.05,
  ...
  theremin: {
    ...
    lfo: { shape: 'sine', rate: 5, amount: 5, target: 'pitch' }, 
    effects: {
      distortion: 0,
  ...
  electricGuitar: {
    ...
    adsr: { attack: 0.02, decay: 0.8, sustain: 0.2, release: 1.0 },
    filter: { type: 'bandpass', cutoff: 1500, q: 3.5 },
    lfo: { shape: 'sine', rate: 0, amount: 0, target: 'pitch' },
    effects: {
      distortion: 0.6,
  ...
  'E-Bells_melody': {
      ...
      filter: { type: 'highpass', cutoff: 800, q: 1 },
      lfo: { shape: 'square', rate: 1.4, amount: 20, target: 'pitch' },
      effects: { 
          distortion: 0, 
          chorus: { rate: 0, depth: 0, mix: 0 }, 
          delay: { time: 0, feedback: 0, mix: 0 } 
  ...
  acousticGuitar: {
    ...
    filter: { type: 'bandpass', cutoff: 2000, q: 4.0 },
    lfo: { shape: 'sine', rate: 0, amount: 0, target: 'pitch' },
    effects: {
      distortion: 0,
  ...
  // Dummy fallback for sampler names to avoid "not found" skip in V1 managers
  blackAcoustic: { layers: [], adsr: {attack:0, decay:0, sustain:0, release:0}, filter: {type:'lowpass', cutoff:0, q:0}, lfo: {shape:'sine', rate:0, amount:0, target:'pitch'}, effects: {distortion:0, chorus:{rate:0,depth:0,mix:0}, delay:{time:0,feedback:0,mix:0}} },
  telecaster: { layers: [], adsr: {attack:0, decay:0, sustain:0, release:0}, filter: {type:'lowpass', cutoff:0, q:0}, lfo: {shape:'sine', rate:0, amount:0, target:'pitch'}, effects: {distortion:0, chorus:{rate:0,depth:0,mix:0}, delay:{time:0,feedback:0,mix:0}} },
};
