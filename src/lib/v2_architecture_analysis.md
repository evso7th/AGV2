...
*   **Несовместимость Гитарных Пресетов:** Пресеты `guitar_*` имели структуру, предназначенную для `type: 'synth'`, но были ошибочно помечены как `type: 'guitar'`. Это приводило к полному игнорированию их параметров, так как гитарная фабрика ожидает совершенно другую структуру (`osc` как объект, а не массив, наличие полей `drive`, `pickup`, `comp` и т.д.).
*   **Отсутствие Фабрики для Органа:** В коде `instrument-factory.ts` реализация для `type: 'organ'` была просто заглушкой (`console.warn`). Это означало, что пресеты органа никогда не могли быть созданы.
*   **Игнорирование Параметров:** Многие детальные параметры в пресетах (например, `noise.color`, `comp` в синтезаторах, `chorus.on`, `delay.on`) не имели соответствующей логики обработки в фабрике и просто игнорировались.
*   **Риски Переподключений:** Фабрика пыталась динамически пересобирать аудио-граф при смене пресетов, что является источником щелчков, утечек памяти и непредсказуемого поведения.

#### 2. Решение: Статический Аудиограф и "Умный Байпас"

Для устранения этих проблем была внедрена новая, более надежная архитектура:

*   **Статический Аудиограф:** Для каждого типа инструмента (`synth`, `guitar`) аудио-цепочка со всеми возможными эффектами (компрессор, фильтры, хорус, дилэй) создается **один раз** при инициализации. Эта цепочка больше никогда не изменяется.
*   **"Умный Байпас" через GainNode:** Вместо переподключения, включение/выключение эффектов теперь управляется через `GainNode`. Каждый эффект обернут в "развилку" `dry/wet`. Если в пресете указано `chorus: { on: false }` или `mix: 0`, фабрика просто устанавливает `wet.gain.value = 0` и `dry.gain.value = 1`, эффективно "обходя" эффект без разрыва цепи.
*   **Полная Реализация Пресетов:** Код фабрики был полностью переписан, чтобы он читал и применял **все** параметры из `presets-v2.ts`, включая настройки компрессора, драйва, эквалайзера и огибающих.
...
3.  **Совместимость Пресетов:** Все пресеты в `presets-v2.ts` теперь на 100% совместимы со структурой, ожидаемой фабрикой. Поле `type` (`'synth'` или `'guitar'`) является ключевым и определяет, какая сборочная линия будет использована.
4.  **Орган и Меллотрон:** Реализации для `type: 'organ'` и `type: 'mellotron'` по-прежнему являются заглушками и требуют отдельной проработки в будущем.

Эталонная версия фабрики находится в коммите, следующем за **Планом 1252**.
