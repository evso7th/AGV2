...
3.  **Обновлен `DarkAmbientBlueprint` (`src/lib/blueprints.ts`):** В части `BUILD` и `MAIN` этого блюпринта добавлены `instrumentRules` для `sfx`, которые увеличивают вероятность их появления до 25% и задают приоритет для категории `voice` (60%).
