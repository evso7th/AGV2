...
**ПРОБЛЕМА:**
Хрипы и падение производительности при игре аккордами на органе. Нагрузка на CPU росла пропорционально количеству нот.

**ПРИЧИНА:**
1.  **Oscillator Overload**: Каждая нота органа создавала 9 независимых осцилляторов (для каждого drawbar). Это приводило к лавинообразному росту активных узлов.
2.  **FX Leakage**: Цепочки эффектов и LFO не полностью изолировались при смене инструментов, оставляя "призрачную" нагрузку на мастер-шину.

**РЕШЕНИЕ:**
1.  **Wavetable Synthesis**: Движок Organ переведен на использование `PeriodicWave`. Теперь суммарный спектр всех drawbar-регистров рассчитывается один раз при смене пресета, и каждая нота использует только **один** оптимизированный осциллятор. Нагрузка снижена в 9 раз.
2.  **Total FX Isolation**: Метод `disconnect()` теперь явно разрывает связи всех внутренних узлов эффектов и останавливает статические LFO.
3.  **Refined Note Lifecycle**: Усилен контроль за транзиентами и огибающими для предотвращения накопления "зомби-узлов".

**РЕЗУЛЬТАТ:**
Достигнута исключительная легкость синтеза. Орган играет чисто и прозрачно даже при плотных аккордах. Общая стабильность системы значительно повышена.
...
3.  **Graph Optimization**: Удален `sumNode` в гитарном конвейере. Множитель 0.175 интегрирован в финальный расчет `sustain` и `peak` огибающей `voiceGain`.
...
2.  **Phase Color**: Внедрен гребенчатый фильтр (Comb Filter) with задержкой 2.5мс для имитации фазового "стекла" Stratocaster.
