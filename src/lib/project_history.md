...
**ДЕЙСТВИЕ:**
Выполнен откат кодовой базы к состоянию до начала интеграции `dark-telecaster-sampler`.

**ПРИЧИНА:**
По словам пользователя, процесс внедрения нового инструмента "полностью убил музыку в меланхоличном блюзе. Звук есть, а музыки нет". Это указывает на критическую потерю музыкальной целостности и атмосферы, даже при формальном отсутствии технических ошибок.

**ЦЕЛЬ ОТКАТА:**
Вернуться к последнему состоянию, где "меланхоличный блюз" обладал необходимой музыкальностью, чтобы переосмыслить подход к добавлению "мрачных" тембров, не разрушая при этом душу композиции.
...
### ЗАПИСЬ: 2024-11-22 (План 1508: "ДНК Сюиты и Паспорт Композитора")

**ПРОБЛЕМА:**
Музыкальные пьесы, особенно в жанре блюз, звучали однообразно. Гармонические прогрессии были статичными, а выбор ритмического стиля — случайным для каждого такта. Это лишало композиции целостности и "души".

**ПРИЧИНА:**
`FractalMusicEngine` не имел долгосрочной "памяти" или "намерения". Он принимал решения на каждом такте, не имея представления об общей структуре и характере пьесы.

**РЕШЕНИЕ (План 1508: "ДНК Сюиты"):**
Была внедрена новая архитектурная концепция — **`SuiteDNA` ("ДНК Сюиты")**.
1.  **"Генетический Код":** В `src/types/fractal.ts` был создан новый тип `SuiteDNA`, который служит уникальным "геномом" для каждой пьесы. Он включает в себя:
    *   `harmonyTrack`: Уникальная гармоническая карта на всю пьесу, сгенерированная с помощью цепей Маркова.
    *   `baseTempo`: "Якорный" темп.
    *   `rhythmicFeel`: Глобальный ритмический "окрас" (шаффл или прямой бит).
    *   `bassStyle` и `drumStyle`: Основные стили исполнения для ритм-секции.
2.  **"Лаборатория":** Функция `generateGhostHarmonyTrack` была переработана в `generateSuiteDNA` (`src/lib/music-theory.ts`). Теперь она генерирует этот полный "геном" один раз в начале каждой сюиты.
3.  **"Паспорт Композитора":** Создан файл `src/lib/dna_composer_guide.md` — руководство, объясняющее, как управлять этой новой системой.
4.  **Интеграция:** `FractalMusicEngine` теперь хранит `SuiteDNA` и использует его для принятия всех ключевых композиторских решений, передавая стилистические команды "исполнителям".

**РЕЗУЛЬТАТ:**
Каждая пьеса теперь обладает уникальным и неизменным характером. Гармония развивается непредсказуемо, но логично. Ритм-секция играет в едином "груве". Это фундаментальное изменение приблизило нас к созданию по-настоящему "живой" и осмысленной музыки.

---

### ЗАПИСЬ: 2024-11-21 (План 1329: "Урок об Утечках")

**ПРОБЛЕМА:**
После серии успешных рефакторингов приложение стало падать через 30-40 секунд фоновой работы на мобильных устройствах. Симптомы включали "спотыкания" ритма, хрипы и последующий крах вкладки браузера.

**ПРИЧИНА:**
Глубокий анализ выявил системную архитектурную ошибку, допущенную мной при предыдущих модификациях, которая затронула восемь сэмплерных модулей (ударные, фортепиано, гитары и т.д.). Внутри метода `schedule` для каждой проигрываемой ноты создавался не только одноразовый `AudioBufferSourceNode`, но и новый узел громкости `GainNode`. Этот узел **никогда не удалялся** из аудио-графа после использования, что приводило к катастрофической утечке аудио-ресурсов.

**РЕШЕНИЕ:**
Была проведена "Генеральная Уборка Аудио-Графа" (План 1328). Для всех восьми проблемных сэмплеров была внедрена корректная логика управления жизненным циклом аудио-узлов:
1.  Узел громкости (`GainNode`) по-прежнему создается для каждой ноты, чтобы обеспечить правильное затухание перекрывающихся звуков.
2.  Ключевым исправлением стало добавление обработчика `onended` к каждому `AudioBufferSourceNode`.
3.  Этот обработчик автоматически вызывает `disconnect()` для соответствующего `GainNode` в тот момент, когда сэмпл заканчивает свое воспроизведение.

**РЕЗУЛЬТАТ:**
Утечка ресурсов полностью устранена. Накопление "аудио-мусора" прекращено. Приложение вновь обрело стабильность и высокую производительность при фоновом воспроизведении на всех устройствах, включая мобильные. Этот инцидент стал одним из важнейших уроков в проекте о необходимости строгого контроля над жизненным циклом ресурсов в Web Audio API.

---

### ЗАПИСЬ: 2024-11-20 (ОТКАТ)

**ДЕЙСТВИЕ:**
Выполнен откат кодовой базы к самой первой версии, восстановленной из GitHub.

**ПРИЧИНА:**
После серии итераций, приведших к архитектурной нестабильности и невозможности решить проблемы с производительностью, было принято стратегическое решение начать разработку с чистого, заведомо стабильного состояния.
...
**РЕШЕНИЕ:**
1.  **Код (`fractal-music-engine.ts`):** В методе `evolve` исправлена строка формирования объекта `instrumentHints`, чтобы он теперь всегда включал вычисленное значение `v2MelodyHint` для партии `melody`.
2.  **Документация (`v2_architecture_analysis.md`):** В архитектурный паспорт добавлен новый раздел, детально описывающий всю цепочку выбора и маршрутизации инструментов, от "заказа" в блюпринте до исполнения конкретным сэмплером.
...
**РЕШЕНИЕ:**
1.  **Создан `NeutralAmbientBlueprint`:** В `src/lib/blueprints.ts` был создан новый тип `SuiteDNA`, который служит уникальным "геномом" для каждой пьесы. Он включает в себя:
    *   `harmonyTrack`: Уникальная гармоническая карта на всю пьесу, сгенерированная с помощью цепей Маркова.
    *   `baseTempo`: "Якорный" темп.
    *   `rhythmicFeel`: Глобальный ритмический "окрас" (шаффл или прямой бит).
    *   `bassStyle` и `drumStyle`: Основные стили исполнения для ритм-секции.
...
3.  **Интеграция:** Новый блюпринт был добавлен в `BLUEPRINT_LIBRARY` под ключом `epic`.
...
2.  **Настройка регистра:** Базовая октавакагаவும் (`musical.key.octave`) для всего `DarkAmbientBlueprint` была понижена с `2` до `1`. Это заставляет сэмплеры подбирать более низкие ноты для всех инструментов, включая скрипку, что естественным образом создает желаемый глубокий, "виолончельный" тембр.
...
**РЕЗУЛЬТАТ:**
В приложении появился новый уникальный стиль "Calm", расширяющий палитру доступных настроений. Стиль генерирует медленную, обволакивающую музыку, идеально подходящую для медитации и релаксации.
...
3.  Там же были добавлены `instrumentRules` для `drums`, предписывающие генератору использовать очень низкую плотность (`density: { min: 0.1, max: 0.3 }`) и флаги, предписывающие использовать преимущественно перкуссию и редкие удары бочки, исключая малый барабан.
