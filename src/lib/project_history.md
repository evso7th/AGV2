...
### ЗАПИСЬ: 2024-12-04 (План №27: "Органная Когезия и Вес")

**ПРОБЛЕМА:**
Пользователь сообщил, что орган по-прежнему звучит "расслоенно" (пэд отдельно, вибрация отдельно), с артефактом "виу-виу" (излишняя частотная модуляция).

**ПРИЧИНА:**
1. Наличие параллельного "сухого" (dry) пути в модуле Лесли создавало эффект статичного пэда, на который сверху накладывался эффект.
2. Глубина Doppler-эффекта (FM) была все еще слишком велика для стабильного восприятия тембра.
3. Отсутствие суб-басового слоя делало орган "тонким", похожим на синтетический пэд.

**РЕШЕНИЕ:**
1. **100% Serial Leslie**: В `instrument-factory.ts` из модуля `createLeslie` полностью удален путь `dry`. Теперь ВЕСЬ сигнал принудительно проходит через роторы. Это склеило звук в единое целое.
2. **Ультра-стабильная Физика**: Глубина FM (Doppler) снижена до `0.0001`, а амплитудная модуляция (AM) — до `0.08`. Это убрало "виу-виу" и создало благородное мерцание.
3. **Гибридный Синтез**: В `buildOrganEngine` добавлена поддержка массива `osc`. В пресеты `organ` и `organ_soft_jazz` внедрен слой суб-баса (`octave: -1`). Это дало инструменту необходимую массу и "телесность".

**РЕЗУЛЬТАТ:**
Достигнута полная монолитность звучания. Орган больше не двоится, звучит глубоко, тепло и вращается как единый акустический объект.
