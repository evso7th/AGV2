...
*   **Ловушка `Tone.PolySynth`:** Первоначальная архитектура использовала `Tone.PolySynth` для воспроизведения аккордов в аккомпанементе. Теоретически, это правильный инструмент. Практически, на мобильных устройствах это приводило к катастрофическим хрипам и артефактов звука. **Урок:** `PolySynth`, как и любой другой инструмент, создающий несколько звуков одновременно, вызывает пиковую нагрузку на CPU. Для критичных к производительности приложений его следует избегать. Любые попытки "оптимизировать" его настройки были обречены, так как проблема была не в настройках, а в самой сути одновременного синтеза.
...
## 5. Протокол "Умной Паузы" (Resilient Pause & Anchor Persistence)

Протокол решает проблему нежелательной смены трека-донора при возобновлении воспроизведения.

### 5.1. Разделение жизненных циклов
Мы четко разделили **создание экземпляра движка** и **выбор музыкального Якоря**. 
*   Движок (Engine) в воркере сохраняется во время паузы. 
*   Якорь (Anchor) фиксируется при инициализации и остается неизменным до конца сюиты или принудительной регенерации.
*   Кнопка Pause теперь только останавливает цикл `tick()`, не уничтожая накопленную память "музыкантов".

### 5.2. Решение Race Condition ("Раннее Рождение")
Web Worker может запуститься до того, как облачные данные (Heritage Axioms) будут загружены.
*   **Решение**: Механизм "Deferred Recognition". Если при старте воркер создал "алгоритмический" движок из-за отсутствия данных, он автоматически проверит наличие аксиом при следующем нажатии Play и проведет "мягкую" переинициализацию под Наследие только один раз.

### 5.3. Ликвидация "Информационных штормов"
Удалены все принудительные вызовы синхронизации данных из UI-потока в момент нажатия Play.
*   **Урок**: Поток данных из Firestore в воркер должен быть строго односторонним и реактивным (через `onSnapshot`). Попытки "подтолкнуть" данные из основного потока приводили к гонке условий и непредсказуемой смене музыкального контекста.
