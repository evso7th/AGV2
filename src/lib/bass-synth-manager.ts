...
/**
 * V1 Bass Synth Manager.
 * Manages multiple synth voices for the bass part using native AudioNodes.
 * This is a lightweight alternative to the V2 factory, ensuring performance on all devices.
 */
export class BassSynthManager {
    ...
    async init() {
        ...
        console.log('[BassSynthManagerV1] Initializing native synth voices for BASS...');
        this.createNoiseBuffer();
        this.initVoicePool(4); // Bass is mostly monophonic, but a small pool helps with releases
        this.isInitialized = true;
        console.log('[BassSynthManagerV1] Native voices initialized.');
    }
    ...
    public schedule(events: FractalEvent[], barStartTime: number, tempo: number, barCount: number, instrumentHint?: BassInstrument, composerControlsInstruments: boolean = true) {
        if (!this.isInitialized) return;
        ...
        if (!instrumentToPlay || instrumentToPlay === 'none' || !(instrumentToPlay in SYNTH_PRESETS)) {
             if (instrumentToPlay && instrumentToPlay !== 'none') {
                console.warn(`[BassSynthManagerV1] Hint "${instrumentToPlay}" not found in V1 SYNTH_PRESETS. Skipping.`);
             }
            return;
        }
